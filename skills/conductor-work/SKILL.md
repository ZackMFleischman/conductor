...
2. Read the ticket and its `assigned_agent_id` (the assigned-to identity). Claim only ready work within scope that is unassigned or assigned to this identity. Do not reassign a ticket to make it claimable. Assignment is not ownership; a session can hold only one active claim.
