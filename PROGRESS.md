...
| T12 · CON-8 · Portal | In progress | portal-coordinator-1 | Build a React and MUI Kanban board with readable task descriptions, assignment, search, and parent filtering. Connect it to real data after the API is ready. | CON-13 live API |
| T13 · CON-13 · Live portal API | Scoped; unclaimed | coordinator-1 | Serve actual project and ticket data and notify the browser when agents change the database, including reconnect recovery. Use a separate backend worktree and coordinate the frontend adapter. | CON-19 contract agreement |
| T14 · CON-19 · Parent hierarchy contract | Ready for review | coordinator-1 | Agree how the API supplies complete parent ancestry so the portal can filter whole ticket subtrees correctly. | — |
