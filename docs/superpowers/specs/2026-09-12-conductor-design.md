...
The claimable queue contains only ready tickets with no active claim, no explicit blocker, and all required prerequisites done. Assigned tickets are claimable only by the matching registered agent identity; unassigned tickets are available to any agent. A resumed agent uses its current claim instead of claiming again.
...
Use a configurable project default for manual QA, copied onto each new ticket; initially default it on. Agents submit a completion summary, validation evidence, and QA instructions into `review`, releasing their claim atomically. Human acceptance changes the ticket to `done`; rejection returns it to `ready` with a reason. Tickets explicitly created without manual QA can finish directly with the same evidence requirements. Agents cannot use routine completion commands to bypass an existing QA requirement.
...
An optional, explicitly invoked `conductor-worker` skill repeatedly claims eligible tickets assigned to its named agent identity or matching user-authorized criteria, runs `conductor-work`, and checks again after each handoff. An empty queue causes a configurable wait (default 30 seconds) before the next check. Atomic filtered claim selection, one active ticket per worker, explicit blocker/release behavior, stop conditions, and session recovery are defined in [Agent integration deliverables](2026-09-12-conductor-agent-deliverables.md). The loop runs inside the user's active agent session and requires no hooks, portal, scheduler, or agent launcher. User-level discovery does not automatically enable autonomous work.

An optional `conductor-orchestrator` skill supervises existing goals/epics, maintains actionable tickets and dependencies, checks worker progress, surfaces human decisions/QA, and uses supported host tools to message or start workers running conductor-worker. It is explicitly scoped and started by the user, with a configurable concurrency cap (default two workers). It reuses idle workers and reconciles pending launches, sessions, and claims rather than spawning on every poll. One project coordinator owns a coordination claim initially. Detailed host capability limits, launch recovery, and stop behavior are in the agent deliverables document. This is a later skill/integration layer; the database and portal do not run an agent scheduler.
...
The retrospective skill supports review-only and fix modes. Review-only saves findings and recommendations. When asked to process reports and fix workflow problems, the agent creates or links improvement tickets, claims the selected work, changes the relevant workflow code, instructions, or tooling within the authorized scope, validates it, and records the result. Do not ask again for routine edits already authorized by a fix request. Fix mode is not blanket permission for unrelated changes or bypassing user/project constraints. Track adoption through linked tickets and append verification or recurrence evidence. A completed improvement ticket means the change was delivered; effectiveness remains unverified until supported by subsequent observations. No permanent reviewing agent, model API, scheduler, or automatic workflow optimizer is needed.
...
The optional worker skill is a separate small increment once atomic filtered selection, claim recovery, and block/release work. It can be used before the portal but is not a prerequisite for shipping the board. The architecture review's recovery and acquisition gaps must be resolved before unattended work is enabled.

The optional orchestrator follows the reliable worker loop and validates one project's supervision through existing host agent tools. It does not delay the tracer or board. Higher-level goals remain epics; cross-provider runtime control is not assumed.
...
This file is a design proposal. No application, installer, or integration has been implemented yet.
