...
import type { BoardSource, BoardTicket, ProblemSummary } from '../board';
...
export function TicketDetails({ ticket, tickets = [], problems = [], source, selectedKey, onClose }: { ticket?: BoardTicket; tickets?: BoardTicket[]; problems?: ProblemSummary[]; source?: BoardSource; selectedKey: string | null; onClose: () => void }) {
  const title = useRef<HTMLHeadingElement>(null);
  const content = useRef<HTMLDivElement>(null);
  const loadComments = useMemo(() => source?.loadTicketNotes && ticket ? (before?: string, signal?: AbortSignal) => source.loadTicketNotes!(ticket.id, before, signal) : undefined, [source, ticket?.id]);
  useEffect(() => {
  ...
  const relatedProblems = ticket ? problems.filter(p => p.ticketKey === ticket.key) : [];
    ...
    <DialogContent ref={content} sx={{ p: 0 }}>
      {!ticket ? <Alert severity="info">This ticket is no longer available on this board.</Alert> : <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'minmax(0, 1fr) 232px' } }}>
        ...
        <Stack component="aside" aria-label="Ticket context" spacing={2.5} sx={{ p: 2.5, bgcolor: '#f7f8fa', borderLeft: { sm: '1px solid' }, borderColor: { sm: 'divider' }, minWidth: 0, gridColumn: { sm: 2 } }}>
          ...
          {relatedProblems.length > 0 && <Box><Typography component="h3" sx={sectionTitle}>Related problems ({relatedProblems.length})</Typography><Stack spacing={1}>{relatedProblems.map(p => <Typography key={p.id} variant="body2" sx={{ overflowWrap: 'anywhere' }}><TicketLink ticketKey={p.key}>{p.key} · {p.summary}</TicketLink></Typography>)}</Stack></Box>}
